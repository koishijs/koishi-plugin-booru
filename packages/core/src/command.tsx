...
import type { Channel, Context, Session, User } from 'koishi'
import type { Config } from '.'

import { Random } from 'koishi'
import { OutputType, preferSizes, sizeNameToFixedWidth, SpoilerType } from '.'
  ...
  const command = ctx
    ...
    .action(async ({ session, options }, query) => {
      ...
      if (query) {
        ...
        const countMatch = /(?:-c|--count)\s+(\d+)/.exec(query)
        if (countMatch) {
          options.count = count(countMatch[1], session)
          query = query.replace(countMatch[0], '').trim()
        }
        const labelMatch = /(?:-l|--label)\s+(\S+)/.exec(query)
        if (labelMatch) {
      ...
      switch (session.resolve(config.outputMethod)) {
        case 'one-by-one':
          // Cordis builds the element tree once and never reconciles it, so keys are unnecessary.
        ...
        case 'forward-multiple':
          if (output.length === 1)
            return <message>{output[0]}</message>
          return (
            <message forward>
              {/* Use <author> for mimicry Bot itself in forward messages (QQ) */}
              <author id={session.userId} name={session.username}></author>
